...
class FollowupService: # Eventually going to take a profile id and derive the context from the db
    ...
    def run(self, about_user_json: str, about_target_json: str) -> str:
        ...
        # If inputs aren't already dicts, parse them as JSON strings (mainly for testing endpoint)
        user_context = about_user_json if isinstance(about_user_json, dict) else json.loads(about_user_json)
        target_context = about_target_json if isinstance(about_target_json, dict) else json.loads(about_target_json)
